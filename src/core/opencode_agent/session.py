"""
Full message session store (OpenCode-style multi-session).
"""
...
def _root() -> Path:
    ...
    env = (os.getenv("SUPERAI_OPENCODE_ROOT") or "").strip()
    if env:
        p = Path(env)
    else:
        p = Path.home() / ".superai" / "opencode_sessions"
...
class OpenCodeSessionStore:
    ...
    def load(self, sid: str) -> SessionState:
        p = self._path(sid)
        if not p.is_file():
            raise KeyError(f"Unknown OpenCode session: {sid}")
        return SessionState.from_dict(json.loads(p.read_text(encoding="utf-8")))

    def list_sessions(self, limit: int = 30) -> List[Dict[str, Any]]:
        rows = []
        for p in sorted(self.root.glob("oc-*.json"), reverse=True)[:limit]:
            try:
    ...
    def undo_last_user_assistant(self, state: SessionState) -> SessionState:
        """Remove last user+assistant exchange if present."""
        if not state.messages:
            return state
        # pop assistant then user if possible
    ...
    def export_markdown(self, state: SessionState, dest: Optional[Path] = None) -> Path:
        lines = [
            f"# SuperAI OpenCode session `{state.id}`",
            "",
            f"- agent: {state.agent}",
        ...
        for m in state.messages:
            ...
            for p in parts:
                if p.get("type") == "tool_call":
                    lines.append(f"\n_tool:_ `{p.get('name')}` `{json.dumps(p.get('arguments') or {})[:200]}`")
